...
import { usePlaces } from "../../context/PlaceContext.jsx";

function Places() {
  const { isOpen, onOpen, onOpenChange } = useDisclosure();
  const { filteredPlaces, loading, error, handleFilterChange } = usePlaces();
    ...
    <div className="flex flex-wrap gap-5 justify-center md:justify-start">
      {filteredPlaces.map((place) => (
        <ReviewCard
          ...
          reviewCount={place.cantReviews}
          reviewSum={place.sumReviews}
          borderColor="blue-700"
          category={"place"}
          imageUrl={place.mainImage || "https://via.placeholder.com/300"}
          minPrice={place.minPrice}
          maxPrice={place.maxPrice}
          location={`${place.street || ""} ${place.streetNumber || ""}, ${
            place.county
          }`}
